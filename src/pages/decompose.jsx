...
// I originally made this body with 800px
// but it's pretty broken on mobile.
// Now it's relative.
const convertWidthPxToPercentage = 100 / 800;
const convertHeightPxToPercentage = 100 / 1300;

const BodyContext = createContext({ fullWidth: 0, fullHeight: 0 });

function Image({ url, x, y, w, h, offsetX, offsetY, scaleX }) {
  const { fullWidth, fullHeight } = useContext(BodyContext);

  const oldToFullWidth = fullWidth / 800;
  const oldToFullHeight = fullHeight / 1300;
...
export default function DecomposePage() {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => {
      ...
      setSize({ width: newWidth, height: (newWidth / 800) * 1300 });
      console.log("hey");
    });
    ...
    <div className="decompose">
      ...
      >
        <BodyContext.Provider
          value={{ fullWidth: size.width, fullHeight: size.height }}
        >
          <Image
